...
public struct SignatureView: View {
    ...
    private let tabTitles = ["Dessiner", "Image", "Saisir"]
    ...
    public var body: some View {
        VStack {
            HStack {
                Button("Enregistrer", action: extractImageAndHandle)
                Spacer()
    ...
    var body: some View {
        Button(action: {
            ...
            if isSet {
                ...
                ZStack {
                    Color.white
                    Text("Choisir une image de signature")
                        .font(.system(size: 18))
                        .foregroundColor(.gray)
                }.frame(height: maxHeight)
                    .overlay(RoundedRectangle(cornerRadius: 4)
...
struct SignatureViewTest: View {
    ...
    var body: some View {
        NavigationView {
            VStack {
                NavigationLink("Aller", destination: SignatureView(onSave: { image in
                    self.image = image
                }, onCancel: {
